...
void EventAction::BeginOfEventAction(const G4Event* evt)
{
 ...
 //   gammsignal = 0;
     ...
     {
         for(int k = 0; k<NOLAYER; k++)
         {
         for(int j = 0; j<NODET; j++)
         {
       totalDetEdep[i][j][k] = 0.;
       LocalTheta[i][j][k] = 0.;
       LocalPhi[i][j][k] = 0.;
         }
         }
...
//    }
    ...
    for (i=0;i<NODET;i++)
    {
        for (j=0;j<NODET;j++)
        {
            for (k=0; k< NOLAYER; k++)
            {
            if (totalDetEdep[i][j][k] > 0.)
            {
    analysisManager->FillNtupleIColumn(1,0, evtNb);
    analysisManager->FillNtupleDColumn(1,1, InitEnergy/keV);
    analysisManager->FillNtupleIColumn(1,2,k);
    analysisManager->FillNtupleIColumn(1,3,i);
    analysisManager->FillNtupleIColumn(1,4,j);
    analysisManager->FillNtupleDColumn(1,5, totalDetEdep[i][j][k]/keV);
    analysisManager->FillNtupleDColumn(1,6, GetStartAngle(0));
        analysisManager->FillNtupleDColumn(1,7, GetStartAngle(1));
    analysisManager->FillNtupleDColumn(1,8, LocalTheta[i][j][k]);
     analysisManager->FillNtupleDColumn(1,9, LocalPhi[i][j][k]);
