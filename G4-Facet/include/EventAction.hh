...
class EventAction : public G4UserEventAction
{
  ...
  public:
    ...
    void AddDetEdep (G4int i, G4int j, G4int k, G4double edep)        { totalDetEdep[i][j][k] += edep;}
...
//    void AddDetESec (G4int i, G4double edep)        { totalDetESec[i] += edep;};
 ...
 //   void SetCollimator(G4int flag) {FoilCame = flag;};
    ...
    void SetStartAngle(G4double angle, G4double phi) {StartAngle[0] = angle, StartAngle[1]=phi;}
    void SetAngle(G4double theta, G4double phi) {Angle[0] = theta, Angle[1]= phi;}
       G4double GetStartAngle(G4int i) {return StartAngle[i];}
       G4double GetAngle(G4int i) {return Angle[i];}
    void AddAngle (G4int i, G4int j, G4int k, G4double theta, G4double phi)  { LocalTheta[i][j][k] = theta, LocalPhi[i][j][k] = phi;}
  ...
  private:
    ...
    static const int NODET = 513;
    static const int NOLAYER = 3;
    static const int NOBINS = 100;
    const double HIST_MAX;
    const double HIST_MIN;
    
    G4double               totalEdep;
    G4double               totalDetEdep[NODET][NODET][NOLAYER];
     G4double               LocalTheta[NODET][NODET][NOLAYER], LocalPhi[NODET][NODET][NOLAYER];
    G4double               Edep[NODET];
    G4double               SumDetEdep;
